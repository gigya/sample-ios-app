...
APIDomain:(NSString *)APIDomain
{
	if ((self = [super init])) {
		...
		if (self.m_pMethod	!=	nil && [self.m_pMethod length]	>	0)
		{
			...
			{
				self.m_pDomain = [NSString stringWithFormat:@"socialize.%@",self.m_APIDomain];
				self.m_pPath =	[NSString stringWithFormat:@"/socialize.%@",self.m_pMethod];
			} else
			{
				NSArray	*pArray = [self.m_pMethod componentsSeparatedByString:@"."];
				self.m_pDomain =	[NSString stringWithFormat:@"%@.%@",[pArray objectAtIndex:0], self.m_APIDomain];
...
-	(void) SendRequest:(id)pContext
{
	...
	[self.m_pRequestParams putStringValue:@"mobile" forKey:@"targetEnv"];
