...
- (void)	onLoginCompleted:(GSObject *)pRequestParams response:(GSResponse *)pResponse context:(id)pContext
{
	...
	if(pResponse	!=	nil)
	{
		if(pResponse.errorCode	==	0)
		{
			...
			if(context != nil && context.loginUIDelegate != nil &&	[context.loginUIDelegate respondsToSelector:@selector(gsLoginUIDidFail:context:)])
			{
				[context.loginUIDelegate	gsLoginUIDidFail:pResponse context:context.context];
			}
		}
...
- (void)	onAddConnectionCompleted:(GSObject *)pParams response:(GSResponse *)pResponse context:(id)pContext
{
	...
	if(pResponse	!=	nil)
	{
		if(pResponse.errorCode	==	0)
		{
			...
			if(context != nil && context.addConnectionsUIDelegate != nil &&	[context.addConnectionsUIDelegate respondsToSelector:@selector(gsAddConnectionsUIDidFail:context:)])
			{
				[context.addConnectionsUIDelegate	gsAddConnectionsUIDidFail:pResponse context:context.context];
			}
		}
		if(context != nil && context.responseDelegate != nil &&	[context.responseDelegate respondsToSelector:@selector(gsDidReceiveResponse:response:context:)])
		{
...
{
	if ((self = [super init])) {
		GSAPI_Internal *pApiInternal	=	[[GSAPI_Internal alloc] initWithAPIKey:apiKey delegate:self];
		self.apiInternal	=	pApiInternal;
		[pApiInternal	release];
	}
    [self setAPIDomain:@"gigya.com"];
	return self;
}
...
-(void) login:(GSObject*)params ViewController:(UIViewController *)MainViewController delegate:(id<GSResponseDelegate>)delegate context:(id)context
{
	if(params	==	nil)
		...
		params	= [params	clone];
	[(GSAPI_Internal	*)self.apiInternal Login:params ViewController:MainViewController context:[GSContext contextWithGSResponseDelegate:delegate context:context]];
}
...
-(void) addConnection:(GSObject*)params ViewController:(UIViewController *)MainViewController delegate:(id<GSResponseDelegate>)delegate context:(id)context
{
	if(params	==	nil)
		...
		params	= 	[params	clone];
	[(GSAPI_Internal	*)self.apiInternal AddConnection:params ViewController:MainViewController  context:[GSContext contextWithGSResponseDelegate:delegate context:context]];
}
...
-(void) showLoginUI:(GSObject*)params ViewController:(UIViewController *)MainViewController delegate:(id<GSLoginUIDelegate>)delegate context:(id)context
{
	if(params	==	nil)
		...
		params	=	[params	clone];
	[(GSAPI_Internal	*)self.apiInternal ShowLoginUI:params ViewController:MainViewController context:[GSContext contextWithGSLoginUIDelegate:delegate context:context]];
}
...
-(void) showAddConnectionsUI:(GSObject *)params ViewController:(UIViewController *)MainViewController delegate:(id<GSAddConnectionsUIDelegate>)delegate context:(id)context
{
	if(params	==	nil)
		...
		params	=	[params	clone];
	[(GSAPI_Internal	*)self.apiInternal ShowAddConnectionsUI:params ViewController:MainViewController context:[GSContext contextWithGSAddConnectionsUIDelegate:delegate context:context]];
}
...
-(void)setAPIDomain:(NSString *)APIDomain {
    [(GSAPI_Internal	*)self.apiInternal SetAPIDomain:APIDomain];
}
@end
