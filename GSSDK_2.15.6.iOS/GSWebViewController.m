...
-(void) NavigateToLogin
{
	...
	if(![self facebookLoginActivated:pProvider] && ![self twitterLoginActivated:pProvider])
	{
		...
		NSString	*pLoginURL	=	[NSString stringWithFormat:GSAPI_LOGIN_URL_HTTPS, self.m_pGSAPI.m_APIDomain];
...
-(void) NavigateToConnect
{
	...
	if(![self facebookLoginActivated:pProvider]	&&	![self twitterLoginActivated:pProvider])
	{
		...
		GSObject *pServerParams= [[[GSObject alloc]init]autorelease];
		NSString	*pConnectURL	=	[NSString stringWithFormat:GSAPI_CONNECT_URL_HTTPS, self.m_pGSAPI.m_APIDomain];
...
-(void) NavigateToProviderSelector
{
    ...
    [self.m_pRequestParams	putStringValue:self.m_pGSAPI.m_pAPIKey forKey:@"apikey"];
	NSURL *pURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@?%@",[NSString stringWithFormat:GSAPI_UI_URL,self.m_pGSAPI.m_APIDomain],[self.m_pRequestParams toQueryString]]];	
	[self.m_pWebView loadRequest:[NSURLRequest requestWithURL:pURL]];
...
-(void) OnResult:(GSObject *)pResponse
{
	...
	if([pResponse getString:GSAPI_PARAM_NAME_ERROR_DESCRIPTION] !=nil)
	{
		...
		self.m_pResponse = [[[GSResponse	alloc]
                                initWithResponseText:[NSString stringWithFormat:@"{\"errorCode\":%d,\"errorMessage\":\"%@\"}",
                                    [[[pArray objectAtIndex:0] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] intValue],
                                    [[pArray objectAtIndex:1] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]]
                                trace:self.m_pTrace] autorelease];
        for (NSString* key in [pResponse getKeys]) {
            NSRange range = [key rangeOfString:@"x_"];
            if (range.location==0) {
                [self.m_pResponse.data putStringValue:[pResponse getString:key] forKey:[key stringByReplacingOccurrencesOfString:@"x_" withString:@""]];
            }
        }
        self.m_pResponse.ResponseText = [self.m_pResponse.data stringValue];
	}	else {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
	...
	if([pURL	rangeOfString:[NSString stringWithFormat:GSAPI_UI_URL,self.m_pGSAPI.m_APIDomain]].location == 0)
	{
		NSMutableDictionary	*pDict = [[[NSMutableDictionary alloc] init] autorelease];
		[pDict	setObject:self.m_pSavedContext forKey:GSAPI_PARAM_NAME_CONEXT];
