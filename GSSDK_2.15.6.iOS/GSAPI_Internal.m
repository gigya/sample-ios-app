...
-	(void) getSDKConfig
{
	...
	@synchronized(self)
	{
		...
		NSURL *pURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://socialize.%@/socialize.getSDKConfig?format=json&apikey=%@",self.m_APIDomain, self.m_pAPIKey]		];
...
-	(void) getTwitterReverseToken
{
	...
	@synchronized(self)
	{
		...
		[NSThread	setThreadPriority:0.01];
			...
			NSURL *pURL = [NSURL URLWithString:[NSString stringWithFormat:@"http://socialize.%@/socialize.getTwitterReverseAuthToken?format=json&apikey=%@", self.m_APIDomain ,self.m_pAPIKey]		];
#ifdef	_DEBUG
...
-	(void) sendErrorReport:(NSArray	*)pParams
{
	...
	@synchronized(self)
	{
		...
		if(bSendReport)
		{
			...
			NSURL *pUrl = [NSURL URLWithString:[NSString stringWithFormat:@"http://socialize.%@/socialize.reportSDKError", self.m_APIDomain]];
...
- (void)setUcidCookie {
    NSDictionary *ucidCookieProps = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                     ...
                                     [NSString stringWithFormat: @".%@", self.m_APIDomain], NSHTTPCookieDomain,
...
- (id)initWithAPIKey:(NSString *)APIKey delegate:(id)pDelegate {
	if ((self = [super init])) {		
		self.m_pAPIKey = APIKey;
		self.delegate	=	pDelegate;
        self.m_APIDomain = @"gigya.com";
		[self LoadSettings];
		[self LoadSession];
        [self setUcidCookie];
...
- (void) OnNotification:(NSNotification *)pNotification
{
	...
	else		if([pNotification.name compare:SendRequestCompleted_Notification] == NSOrderedSame)
	{
        ...
        GSObject *oSession;
        if ([pResponse.data contains:@"sessionInfo"]){
            oSession = [pResponse getObject:@"sessionInfo"];
        } else {
            oSession = pResponse.data;
        }
        if ([oSession contains:@"sessionToken"]) {
            if ([oSession contains:@"expires_in"]) {
                [self SetSession:[[[GSSession alloc] initWithSessionToken:[oSession getString:@"sessionToken"] sessionSecret:[oSession getString:@"sessionSecret"] expirationTime:[NSDate dateWithTimeIntervalSinceNow:[[oSession getString:@"expires_in"] intValue]]] autorelease]];
            } else {
                [self SetSession:[[[GSSession alloc] initWithSessionToken:[oSession getString:@"sessionToken"] sessionSecret:[oSession getString:@"sessionSecret"]] autorelease]];
            }
...
-(void) SaveSession
{
	...
	{
		if (self.m_pSession.secret != nil) [self.m_pSettings	setObject:self.m_pSession.secret forKey:@"session.Secret"];
		if (self.m_pSession.accessToken != nil) [self.m_pSettings	setObject:self.m_pSession.accessToken forKey:@"session.Token"];
		if (self.m_pSession.expirationTime != nil) [self.m_pSettings	setObject:self.m_pSession.expirationTime forKey:@"session.ExpirationTime"];
	}	else {
...
- (void)GSWebViewControllerDidFinish:(UIViewController *)controller {
    ...
    int errorCode = pWebViewController.m_pResponse.errorCode;
	if(errorCode ==	0)
	{
...
-	(void) SendRequest:(NSString	*)pMethod Params:(GSObject		*)pParams	UseHTTPS:(BOOL)bUseHTTPS context:(id)pContext	trace:(GSLogger	*)pTrace
{
	...
	*/
	...
	if(![self.m_pSession IsValid])
	{
		pRequest = [[GSRequest alloc] initWithSessionToken:self.m_pAPIKey SecretKey:nil APIMethod:pMethod SendParams:pParams UseHTTPS:bUseHTTPS NotificationMessage:self.m_pNotificationMessage trace:pTrace serverTimestampSkewDelegate:self APIDomain:self.m_APIDomain];
	}	else {
		pRequest = [[GSRequest alloc] initWithSessionToken:[self GetAccessToken] SecretKey:[self GetSecretKey] APIMethod:pMethod SendParams:pParams UseHTTPS:bUseHTTPS NotificationMessage:self.m_pNotificationMessage trace:pTrace serverTimestampSkewDelegate:self APIDomain:self.m_APIDomain];
	}
